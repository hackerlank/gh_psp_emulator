...
int ADPCM_sh_start (struct ADPCMinterface *intf)
{
	...
	intf->frequency=(int)(((100.0-((snd_pcm_reduction-1.0)*5.0))*((double)intf->frequency))/100.0);
	...
	if (intf->init)
	{
		sample_list = (ADPCMsample*)gp2x_malloc (257 * sizeof (struct ADPCMsample));
		if (!sample_list)
			return 1;
		fast_memset (sample_list, 0, 257 * sizeof (struct ADPCMsample));
	...
	fast_memset (adpcm, 0, sizeof (adpcm));
	for (i = 0; i < intf->num; i++)
	{
		...
		adpcm[i].buffer = gp2x_malloc (buffer_len * Machine->sample_bits / 8);
		if (!adpcm[i].buffer)
			return 1;
	}
...
void ADPCM_sh_stop (void)
{
	...
	{
		if (adpcm[i].stream)
			gp2x_free (adpcm[i].stream);
		if (adpcm[i].buffer)
			gp2x_free (adpcm[i].buffer);
	}
}
...
static void ADPCM_update (struct ADPCMVoice *voice, int finalpos)
{
	...
	if (left > 0)
	{
		...
		if (voice->playing)
		{
			...
			if (Machine->sample_bits == 16)
			{
				...
				while (left)
				{
					...
					val = base[(sample>>1) & mask] >> (((sample & 1) << 2) ^ 4);
					signal += diff_lookup[(step << 4) + (val & 15)];
		...
		{
			if (Machine->sample_bits == 16)
				fast_memset ((short *)voice->buffer + voice->bufpos, 0, left << 1);
			else
				fast_memset ((unsigned char *)voice->buffer + voice->bufpos, AUDIO_CONV (0), left);
		}
...
void ADPCM_sh_update (void)
{
	...
	for (i = 0; i < adpcm_intf->num; i++)
	{
		...
		/* play the result */
		if (Machine->sample_bits == 16)
			osd_play_streamed_sample_16 (voice->channel, voice->buffer, buffer_len<<1, emulation_rate, voice->volume,OSD_PAN_CENTER);
		else
			osd_play_streamed_sample (voice->channel, (signed char *)voice->buffer, buffer_len, emulation_rate, voice->volume,OSD_PAN_CENTER);
...
int MSM5205_sh_start (struct MSM5205interface *intf)
{
	...
	if (!result)
	{
		...
		for (i = 0; i < intf->num; i++)
		{
			...
			voice->stream = (unsigned char*)gp2x_malloc (stream_size);
			if (!voice->stream)
				return 1;
