...
void phoenix_sound_control_a_w(int offset,int data)
{
	...
	if (freq != 0x0f)
	{
		sample_adjust(0,MAXFREQ_A/(16-sound_a_freq),MAX_VOLUME*(3-vol));
		sound_a_play = 1;
	}
	else
	{
		sample_adjust(0,SAFREQ,0);
		sound_a_play = 0;
	}
	...
	if (noisemulate)
	{
		...
		if (noise) sample_adjust(2,noise_freq,noise_vol/4);
		else
		{
			sample_adjust(2,1000,0);
			noise_vol = 0;
		}
	}
	else
...
void phoenix_sound_control_b_w(int offset,int data)
{
	...
	if (freq != 0x0f)
	{
		sample_adjust(1,MAXFREQ_B/(16-sound_b_freq),MAX_VOLUME*(3-vol));
		sound_b_play = 1;
	}
	else
	{
		sample_adjust(1,SBFREQ,0);
		sound_b_play = 0;
	}
...
int phoenix_sh_start(void)
{
	...
	sample_setup(0,(signed char*)waveform1,32,1000,0,1);
	sample_setup(1,(signed char*)waveform1,32,1000,0,1);
	sample_setup(2,(signed char*)waveform2,128,1000,0,1);
	song_playing = 0;
...
/*	if (sound_a_adjust)
	...
	if (sound_a_play)
		sample_adjust(0,pitch_a,MAX_VOLUME*(3-sound_a_vol));
	if (sound_b_play)
		sample_adjust(1,pitch_b,MAX_VOLUME*(3-sound_b_vol));

	if ((noise_vol) && (noisemulate))
	{
		sample_adjust(2,noise_freq,noise_vol/4);
		noise_vol-=3;
	}
