...
static void VLM5030_reset(void)
{
	...
	fast_memset(old_k, 0, sizeof(old_k));
	fast_memset(new_k, 0, sizeof(new_k));
	fast_memset(current_k, 0, sizeof(current_k));
	fast_memset(target_k, 0, sizeof(target_k));
	interp_count = sample_count = pitch_count = 0;
	fast_memset(x, 0, sizeof(x));
	/* reset parameters */
...
int VLM5030_sh_start(struct VLM5030interface *interface)
{
	...
	channel = stream_init(VLM_NAME, emulation_rate, Machine->sample_bits,0,(void (*)(int,void*,int))vlm5030_update_callback);
	if (channel == -1) return 1;

	//schannel = get_play_channels(2);
	return 0;
}
