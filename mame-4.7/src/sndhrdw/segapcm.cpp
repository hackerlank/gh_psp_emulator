...
/*                                              */
/************************************************/
int SEGAPCM_sh_start( const struct SEGAPCMinterface *intf )
{
	if (Machine->sample_rate == 0) return 0;
	if( SEGAPCMInit( intf->bank&0x00ffffff, intf->mode, (unsigned char *)Machine->memory_region[intf->region]+0x10000, intf->volume ) )
		return 1;
	return 0;
}
/************************************************/
...
/*    initial SEGAPCM                           */
/************************************************/
int SEGAPCMInit( int banksize, int mode, unsigned char *inpcm, int volume )
{
	int i;
...
void SEGAPCMUpdate( int num, void **buffer, int length )
{
	...
	fast_memset( datap[0], 0x00, length * sizeof(SEGAPCM_SMP) );
	fast_memset( datap[1], 0x00, length * sizeof(SEGAPCM_SMP) );
