...
void YM2203UpdateOne(int num, void *buffer, int length)
{
    	...
    	do
	{
		...
		*buf++ = (Limit( outd[OPN_CENTER] , OPN_MAXOUT, OPN_MINOUT ) >> OPN_OUTSB);
	} while (--length);
}
...
void YM2610UpdateOne(int num, void **buffer, int length)
{
    	...
    	do
	{
		...
		/* buffering */
		*bufL++ = (Limit( outd[OPN_CENTER] + outd[OPN_LEFT], OPNB_MAXOUT, OPNB_MINOUT ))>>OPNB_OUTSB;
		*bufR++ = (Limit( outd[OPN_CENTER] + outd[OPN_RIGHT], OPNB_MAXOUT, OPNB_MINOUT ))>>OPNB_OUTSB;
...
void OPMUpdateOne(int num, void **buffer, int length)
{
    	...
    	do
	{
		...
		FM_CALC_CH( cch[7] );
		/* stereo separate */
		*bufL++ = (Limit( outd[OPM_CENTER] + outd[OPM_LEFT], OPM_MAXOUT, OPM_MINOUT ))>>OPM_OUTSB;
		*bufR++ = (Limit( outd[OPM_CENTER] + outd[OPM_RIGHT], OPM_MAXOUT, OPM_MINOUT ))>>OPM_OUTSB;
    	} while (--length);
}
