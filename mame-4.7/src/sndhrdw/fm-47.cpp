...
static int FMInitTable( void )
{
	...
	TL_TABLE = (int *)gp2x_malloc(TL_MAX*2*sizeof(int));
...
{
	if( TL_TABLE ) gp2x_free( TL_TABLE );
	return;
}
...
void YM2203UpdateOne(int num, void *buffer, int length)
{
    	...
    	do
	{
		...
		FM_CALC_CH( cch[2] );
		/* Limit check */
		Limit( outd[OPN_CENTER] , OPN_MAXOUT, OPN_MINOUT );
		/* store to sound buffer */
		*buf++ = outd[OPN_CENTER] >> OPN_OUTSB;
	} while (--length);
}
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	if( (FM2203 = (YM2203 *)gp2x_malloc(sizeof(YM2203) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	fast_memset(FM2203,0,sizeof(YM2203) * FMNumChips);
...
void YM2608UpdateOne(int num, void **buffer, int length)
{
   	...
   	do
	{
		...
		FM_CALC_CH( cch[5] );
		/* stereo separate */
		outd[OPN_LEFT] += outd[OPN_CENTER];
		Limit( outd[OPN_LEFT], OPN_MAXOUT, OPN_MINOUT );
		*bufL++ = ( outd[OPN_LEFT] >> OPN_OUTSB );
		outd[OPN_RIGHT] += outd[OPN_CENTER];
		Limit( outd[OPN_RIGHT], OPN_MAXOUT, OPN_MINOUT );
		*bufR++ = ( outd[OPN_RIGHT] >> OPN_OUTSB );
	} while (--length);
}
...

{
	...
	if( (FM2608 = (YM2608 *)gp2x_malloc(sizeof(YM2608) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	fast_memset(FM2608,0,sizeof(YM2608) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gp2x_free( FM2608 );
...
INLINE void OPNB_ADPCM_CALC_CHB( YM2610 *F2610, ADPCM_CH *ch )
{
	...
	if ( ch->now_step >= (1<<ADPCM_SHIFT) )
	{
		...
		do{
			...
			ch->adpcmx += (decode_tableB1[data] * ch->adpcmd / 8);
			Limit ( ch->adpcmx, ADPCMB_DECODE_MAX, ADPCMB_DECODE_MIN );
			/* delta to next delta */
			ch->adpcmd = ( ch->adpcmd * decode_tableB2[data] ) / 64;
			Limit( ch->adpcmd, ADPCMB_DELTA_MAX, ADPCMB_DELTA_MIN );
...
void YM2610UpdateOne(int num, void **buffer, int length)
{
    	...
    	do
	{
		...
		FM_CALC_CH( cch[4] );
		FM_CALC_CH( cch[5] );
		if( F2610->adpcm[0].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[0]);
		if( F2610->adpcm[1].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[1]);
		if( F2610->adpcm[2].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[2]);
		if( F2610->adpcm[3].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[3]);
		if( F2610->adpcm[4].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[4]);
		if( F2610->adpcm[5].flag )
			OPNB_ADPCM_CALC_CHA( F2610, &F2610->adpcm[5]);
		/* buffering */
		outd[OPN_LEFT] += outd[OPN_CENTER];
		Limit( outd[OPN_LEFT], OPNB_MAXOUT, OPNB_MINOUT );
		*bufL++ = ( outd[OPN_LEFT] >> OPNB_OUTSB );
		outd[OPN_RIGHT] += outd[OPN_CENTER];
		Limit( outd[OPN_RIGHT], OPNB_MAXOUT, OPNB_MINOUT );
		*bufR++ = ( outd[OPN_RIGHT] >> OPNB_OUTSB );
	} while (--length);
}
...

{
	...
	/* allocate extend state space */
	if( (FM2610 = (YM2610 *)gp2x_malloc(sizeof(YM2610) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	fast_memset(FM2610,0,sizeof(YM2610) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gp2x_free( FM2610 );
	...
	for ( i = 0 ; i < FMNumChips; i++ ) {
		...
		/* ADPCM */
#ifndef __RAINE__
		FM2610[i].pcmbuf[0] = (char *)(Machine->memory_region[pcmroma[i]]);
		FM2610[i].pcm_size[0] = Machine->memory_region_length[pcmroma[i]];
...
/* 10-1f */
void YM2610ADPCMWrite1(int n,int r,int v)
{
	...
	case 0x13:	/* Start Address H */
	  adpcm->start  = (F2610->adpcmreg[0][0x13]*0x0100 | F2610->adpcmreg[0][0x12]) << F2610->port0shift;
		break;
	case 0x14:	/* Stop Address L */
	case 0x15:	/* Stop Address H */
		adpcm->end    = (F2610->adpcmreg[0][0x15]*0x0100 | F2610->adpcmreg[0][0x14]) << F2610->port0shift;
...
/* 100-12f */
void YM2610ADPCMWrite2(int n,int r,int v)
{
	YM2610 *F2610 = &(FM2610[n]);
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	/* allocate extend state space */
	if( (FM2612 = (YM2612 *)gp2x_malloc(sizeof(YM2612) * FMNumChips))==NULL)
		return (-1);
	/* clear */
	fast_memset(FM2612,0,sizeof(YM2612) * FMNumChips);
	/* allocate total level table (128kb space) */
	if( !FMInitTable() )
	{
		gp2x_free( FM2612 );
...
int YM2612Write(int n, int a,int v)
{
	...
	case 3:	/* data port 1    */
		...
		YM2612UpdateReq(n);
#ifdef FM_DEBUG
		if( addr == 0x28 ) OPNWriteMode(&(F2612->OPN),addr,v|0x04);
		else
#endif
...
void OPMInitTable( int num )
{
	...
	if (FMOPM[num].ST.rate)
		rate = (double)(1<<FREQ_BITS) / ((3579545.0/snd_opm_reduction) / FMOPM[num].ST.clock * FMOPM[num].ST.rate);
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	if( (FMOPM = (YM2151 *)gp2x_malloc(sizeof(YM2151) * FMNumChips))==NULL)
		return (-1);
...
void OPMUpdateOne(int num, void **buffer, int length)
{
    	...
    	do
	{
		...
		/* stereo separate */
		outd[OPN_LEFT] += outd[OPN_CENTER];
		Limit( outd[OPN_LEFT], OPM_MAXOUT, OPM_MINOUT );
		*bufL++ = ( outd[OPN_LEFT] >> OPM_OUTSB );
		outd[OPN_RIGHT] += outd[OPN_CENTER];
		Limit( outd[OPN_RIGHT], OPM_MAXOUT, OPM_MINOUT );
		*bufR++ = ( outd[OPN_RIGHT] >> OPM_OUTSB );
    	} while (--length);
}
