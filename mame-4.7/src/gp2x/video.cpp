...
#ifdef GP2X
	if (gp2x_showfps) {
		fast_memset(&gp2x_screen8[0*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[1*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[2*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[3*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[4*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[5*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[6*gp2x_text_width],0,48);
		fast_memset(&gp2x_screen8[7*gp2x_text_width],0,48);
	}
...
INLINE void update_timing (void)
{
	...
	unsigned long interval=(((unsigned long)((frameskip+1)*1000)) * gp2x_timer_read_scale())/((unsigned long)(Machine->drv->frames_per_second));
