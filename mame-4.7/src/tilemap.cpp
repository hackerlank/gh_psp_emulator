...
static void dispose_tile_info( struct tilemap *tilemap ){
	gp2x_free( tilemap->pendata );
	gp2x_free( tilemap->maskdata );
	gp2x_free( tilemap->paldata );
	gp2x_free( tilemap->pen_usage );
	gp2x_free( tilemap->priority );
	gp2x_free( tilemap->visible );
	gp2x_free( tilemap->dirty_vram );
	gp2x_free( tilemap->dirty_pixels );
	gp2x_free( tilemap->flags );
	gp2x_free( tilemap->priority_row );
	gp2x_free( tilemap->visible_row );
}
...
static int create_tile_info( struct tilemap *tilemap ){
	...
	tilemap->pendata = (unsigned char **)gp2x_malloc( sizeof( unsigned char *)*num_tiles );
	tilemap->maskdata = (unsigned char **)gp2x_malloc( sizeof( unsigned char *)*num_tiles ); /* needed only for TILEMAP_BITMASK */
	tilemap->paldata = (unsigned short **)gp2x_malloc( sizeof( unsigned short *)*num_tiles );
	tilemap->pen_usage = (unsigned int *)gp2x_malloc( sizeof( unsigned int )*num_tiles );
	tilemap->priority = (char *)gp2x_malloc( num_tiles );
	tilemap->visible = (char *)gp2x_malloc( num_tiles );
	tilemap->dirty_vram = (char *)gp2x_malloc( num_tiles );
	tilemap->dirty_pixels = (char *)gp2x_malloc( num_tiles );
	tilemap->flags = (unsigned char *)gp2x_malloc( num_tiles );
	tilemap->rowscroll = (int *)gp2x_malloc(tilemap->height * sizeof(int));
	tilemap->colscroll = (int *)gp2x_malloc(tilemap->width * sizeof(int));
	fast_memset(tilemap->rowscroll, 0, tilemap->height * sizeof(int));
	fast_memset(tilemap->colscroll, 0, tilemap->width * sizeof(int));

	tilemap->priority_row = (char **)gp2x_malloc( sizeof(char *)*num_rows );
	tilemap->visible_row = (char **)gp2x_malloc( sizeof(char *)*num_rows );
		...
		tilemap->priority_row && tilemap->visible_row )
	{
		...
		fast_memset( tilemap->priority, 0, num_tiles );
		fast_memset( tilemap->visible, 0, num_tiles );
		fast_memset( tilemap->dirty_vram, 1, num_tiles );
		fast_memset( tilemap->dirty_pixels, 1, num_tiles );
...
static void dispose_pixmap( struct tilemap *tilemap ){
	...
	gp2x_free( tilemap->colscroll );
	gp2x_free( tilemap->rowscroll );
}

static unsigned char **new_mask_data_table( unsigned char *mask_data, int num_cols, int num_rows ){
	unsigned char **mask_data_row = (unsigned char **)gp2x_malloc(num_rows * sizeof(unsigned char *));
	if( mask_data_row ){
		int row;
...
static int create_fg_mask( struct tilemap *tilemap ){
	...
	tilemap->fg_mask_data = (unsigned char *)gp2x_malloc( tilemap->num_tiles );
	if( tilemap->fg_mask_data ){
		...
		if( tilemap->fg_mask_data_row ){
			...
			gp2x_free( tilemap->fg_mask_data_row );
		}
		gp2x_free( tilemap->fg_mask_data );
	}
	return 0; /* error */
}
...
static int create_bg_mask( struct tilemap *tilemap ){
	...
	tilemap->bg_mask_data = (unsigned char *)gp2x_malloc( tilemap->num_tiles );
	if( tilemap->bg_mask_data ){
		...
		if( tilemap->bg_mask_data_row ){
			...
			gp2x_free( tilemap->bg_mask_data_row );
		}
		gp2x_free( tilemap->bg_mask_data );
	}
	return 0; /* error */
}
...
struct tilemap *tilemap_create(
	...
	struct tilemap *tilemap = (struct tilemap *)gp2x_malloc(sizeof( struct tilemap ) );
	fast_memset(tilemap, 0, sizeof( struct tilemap ));
	if( tilemap ){
		if( orientation & ORIENTATION_SWAP_XY ){
...
void tilemap_mark_all_tiles_dirty( struct tilemap *tilemap ){
	fast_memset( tilemap->dirty_vram, 1, tilemap->num_tiles );
}
...
void tilemap_mark_all_pixels_dirty( struct tilemap *tilemap ){
	if( tilemap==ALL_TILEMAPS ){
		...
		fast_memset( tilemap->dirty_pixels, 1, tilemap->num_tiles );
	}
}
...
void tilemap_update( struct tilemap *tilemap ){
	if( tilemap==ALL_TILEMAPS ){
		...
		if( tilemap->scrolled ){
			...
			fast_memset( tilemap->visible, 0, tilemap->num_tiles );
