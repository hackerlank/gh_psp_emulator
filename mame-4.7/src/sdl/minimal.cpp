...
void gp2x_init(int tickspersecond, int bpp, int rate, int bits, int stereo, int hz)
{
...
#ifdef PSP_RES
	gp2x_sdlwrapper_screen=SDL_SetVideoMode(480,240,16,flags);
#else
...
#ifdef PSP_RES
			gp2x_sdlwrapper_screen_pixels=malloc(480*240*bpp>>3);
#else
	...
	gp2x_sdlwrapper_ticksdivisor=((double)tickspersecond)*1000.0;
#endif
