...
// DIV2
		...
		gp2x_width=960;
		gp2x_height=480;
		if (visible_game_width>960) 
			gp2x_xoffset = (visible_game_width-960) / 2;
		else {
			gp2x_width = visible_game_width;
			/* fix to correct display of <480 vertical games, like Popeye, Super Sprint etc. */
			/* maybe a wrong centering routine? */
			(visible_game_height>479)?gp2x_xscreen=(960-visible_game_width)/2:gp2x_xscreen=(visible_game_width-1088)/2;
		}
#else
		gp2x_width=640;
		gp2x_height=480;
...
//		if ((visible_game_width>visible_game_height)&&(gp2x_ymax<=240))
...
#endif
	{
		...
		int gp2x_width_tst=480;
		gp2x_width=gp2x_width_tst;
		gp2x_height=240;
		if (visible_game_width>gp2x_width_tst){
			gp2x_xoffset = (visible_game_width-gp2x_width_tst) / 2 ;
		}
		else {
			gp2x_width = visible_game_width;
			/* fix to correct disply of <240 vertical games, like Bubble Bobble, Rygar, City Connection etc. */
			/* maybe a wrong centering routine? */
			(visible_game_height>239)?gp2x_xscreen=(480-visible_game_width)/2:gp2x_xscreen=(visible_game_width-544)/2;
		}
		gp2x_xoffset+= visible_game_width_offset;
...
#ifdef GP2X
	if (gp2x_showfps) {
		memset(&gp2x_screen8[0*gp2x_text_width],0,40);
		memset(&gp2x_screen8[1*gp2x_text_width],0,40);
		memset(&gp2x_screen8[2*gp2x_text_width],0,40);
		memset(&gp2x_screen8[3*gp2x_text_width],0,40);
		memset(&gp2x_screen8[4*gp2x_text_width],0,40);
		memset(&gp2x_screen8[5*gp2x_text_width],0,40);
		memset(&gp2x_screen8[6*gp2x_text_width],0,40);
		memset(&gp2x_screen8[7*gp2x_text_width],0,40);
	}
...
INLINE void frame_counter (void)
{
	...
	static unsigned int gp2x_fc_t0; /* Last T0 */
	unsigned int gp2x_fc_t1=gp2x_timer_read();
	char texto[8];
	gp2x_fc++;
	if (gp2x_fc_t1-gp2x_fc_t0>1000) {
		gp2x_fc_t0=gp2x_fc_t1;
		gp2x_fc_v=gp2x_fc;
		gp2x_fc=0;
	}
	if(gp2x_fc_v>Machine->drv->frames_per_second)
		gp2x_fc_v=Machine->drv->frames_per_second;
	sprintf(texto,"%d/%d",gp2x_fc_v,Machine->drv->frames_per_second);
	memset(&gp2x_screen8[0*gp2x_text_width],0,40);
	memset(&gp2x_screen8[1*gp2x_text_width],0,40);
	memset(&gp2x_screen8[2*gp2x_text_width],0,40);
	memset(&gp2x_screen8[3*gp2x_text_width],0,40);
...
static void update_display (void)
{
	...
	unsigned int height=gp2x_height;

	do {
		fast_memcpy(buffer_scr,buffer_mem,width);
		buffer_scr+=320;			
...
static void update_display_scaled (void)
{
	...
	unsigned int height=visible_game_height;
...
static void update_display_scaled_rotated (void) 
{
	...
	int x,y;
	const int width=visible_game_width;
...
static update_display_func update_display_array[4]=
{
