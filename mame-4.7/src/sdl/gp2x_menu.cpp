...
#ifdef PSP_RES
#ifdef TTY_BMP
#include "gfx/pspmenu480.h"
#include "gfx/pspsplash480.h"
#else
...
void gp2x_intro_screen(void) {
	...
	if (f) {
#ifdef PSP_RES
		fread(gp2xsplash_bmp,1,1078+480*240,f);
#else
	...
	if (f) {
#ifdef PSP_RES
		fread(gp2xmenu_bmp,1,1078+480*240,f);
#else
...
#define SAVEDATASTRUCTURE "gp2x_freq=%d;gp2x_frameskip=%d;gp2x_frameskip_auto=%d;gp2x_clock_cpu=%d;gp2x_sound_enable=%d;gp2x_clock_sound=%d;key_setting=%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%d,%d,%d,%d;gp2x_double_buffer=%d;gp2x_vsync=%d;gp2x_autofire=%d,%d,%d;\0"
#else
...
#ifdef PSP
		/* PSP Clock*/
		gp2x_gamelist_text_out_fmt(x_Pos,y_Pos+110, "PSP Clock     %d MHz", gp2x_freq);
		gp2x_gamelist_text_out(x_Pos,y_Pos+120,"Press [] to confirm, X return\0");
#else
		gp2x_gamelist_text_out(x_Pos,y_Pos+120,"Press A to confirm, B return\0");
#endif
            ...
            case 8:
				...
				if(ExKey & GP2X_RIGHT){
					switch (gp2x_freq) {
						...
						case 333: gp2x_freq=166;break;
					}
				} else {
					switch (gp2x_freq) {
						case 166: gp2x_freq=333;break;
...
#ifdef PSP
	(gp2x_freq<222)?scePowerSetBusClockFrequency(gp2x_freq/2)&&scePowerSetCpuClockFrequency(gp2x_freq):scePowerSetClockFrequency(gp2x_freq,gp2x_freq,166);
#else
