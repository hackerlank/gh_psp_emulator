#ifndef MAME_OSDEPEND_H
#define MAME_OSDEPEND_H
...
#ifdef GP2X
#define gp2x_malloc(x) gp2x_mallocX(x, __FUNCTION__)
#define gp2x_free(x) gp2x_freeX(x, __FUNCTION__)
#endif
...
struct osd_bitmap *osd_create_display(int width,int height,int attributes,
									  int visible_width, int visible_height,int visible_width_offset, int visible_height_offset);
...
#define osd_play_sample(channel,data,len,freq,volume,loop) playsample(channel,data,len,freq,volume,loop,OSD_PAN_CENTER,8)
#define osd_play_sample_16(channel,data,len,freq,volume,loop) playsample(channel,(signed char *)data,len,freq,volume,loop,OSD_PAN_CENTER,16)
#define osd_play_streamed_sample(channel,data,len,freq,volume,pan) playsample(channel,data,len,freq,volume,1,pan,8)
#define osd_play_streamed_sample_16(channel,data,len,freq,volume,pan) playsample(channel,(signed char *)data,len,freq,volume,1,pan,16)
...
*/
#define osd_ym2203_write(n,r,v)
#define osd_ym2203_update()
#define osd_ym3812_control(reg)
#define osd_ym3812_write(data)

/* FRANXIS 10-08-2005 */
/*
int osd_get_mastervolume(void);
*/
#define osd_get_mastervolume() 0
