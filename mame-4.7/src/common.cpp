#define COMMON_ACORN_PATCH
#define COMMON_TOTALS_PATCH
...
#define read_dword(addr) (((int)addr & 3)? ( ((((unsigned char *)(addr))[BL0])) | ((((unsigned char *)(addr))[BL1]) <<  8) | ((((unsigned char *)(addr))[BL2]) << 16) | ((((unsigned char *)(addr))[BL3]) << 24) ) : (*(int *)addr) )
#define write_dword(addr,value) (((int)addr & 3)? ( ((unsigned char *)(addr))[BL0] = (unsigned char)((int)(value)), ((unsigned char *)(addr))[BL1] = (unsigned char)((int)(value) >>  8), ((unsigned char *)(addr))[BL2] = (unsigned char)((int)(value) >> 16), ((unsigned char *)(addr))[BL3] = (unsigned char)((int)(value) >> 24) ) : (*(int *)addr = value) )
#ifdef COMMON_ACORN_PATCH
#define read_dword_dword(address) *(int *)address
#define write_dword_dword(address,data) *(int *)address=data
#endif
#else
...
int readroms(void)
{
	...
	while (romp->name || romp->offset || romp->length)
	{
		...
		if (romp->name || romp->length)
		{
			...
			gp2x_text_log("Error in RomModule definition: expecting ROM_REGION");
			goto getout;
		}
		region_size = romp->offset & ~ROMFLAG_MASK;
		if ((Machine->memory_region[region] = (unsigned char*)gp2x_malloc(region_size)) == 0)
		{
			/*printf("readroms():  Unable to allocate %d bytes of RAM\n",region_size);*/
			/*gp2x_text_log("Unable to allocate RAM");*/
			gp2x_text_log_fmt("Unable to allocate %d bytes of RAM",region_size);
			goto getout;
		}
		Machine->memory_region_length[region] = region_size;

		/* some games (i.e. Pleiades) want the memory clear on startup */
		fast_memset(Machine->memory_region[region],0,region_size);
		...
		while (romp->length)
		{
			...
			if (romp->name == 0)
			{
				...
				gp2x_text_log("Error in RomModule definition: ROM_CONTINUE not preceded by ROM_LOAD");
				goto getout;
			}
			else if (romp->name == (char *)-1)
			{
				/*printf("Error in RomModule definition: ROM_RELOAD not preceded by ROM_LOAD\n");*/
				gp2x_text_log("Error in RomModule definition: ROM_RELOAD not preceded by ROM_LOAD");
				goto getout;
			}
			...
			{
				/* Unable to open ROM */
				gp2x_text_log("Unable to open ROM");
				goto getout;
			}
			...
			do
			{
				...
				if (romp->offset + length > region_size)
				{
					...
					gp2x_text_log("Error in RomModule definition: out of memory region space");
					osd_fclose(f);
					goto getout;
				}
				...
				if (romp->length & (ROMFLAG_ALTERNATE | ROMFLAG_NIBBLE))
				{
					...
					temp = (unsigned char *)gp2x_malloc (length);

					if (!temp)
					{
						/*printf("Out of memory reading ROM %s\n",name);*/
						/*gp2x_text_log("Out of memory reading ROMs");*/
						gp2x_text_log_fmt("Out of memory reading ROM %s",name);
						osd_fclose(f);
						goto getout;
					}
					...
					if (osd_fread(f,temp,length) != length)
					{
						...
						/*gp2x_text_log("Unable to read ROM");*/
						gp2x_text_log_fmt("Unable to read ROM %s",name);
						gp2x_free(temp);
						osd_fclose(f);
						goto getout;
					}
			...
			if (explength != osd_fsize (f))
			{
				...
				/*gp2x_text_log("Length mismatch on ROM");*/
				gp2x_text_log_fmt("Length mismatch on ROM %s",name);
				lengthwarning++;
			}
			...
			if (expchecksum != osd_fcrc (f))
			{
				...
				if (checksumwarning == 0)
					/*printf ("The checksum of some ROMs does not match that of the ones MAME was tested with.\n"
							...
							"Name         Expected  Found\n");*/
					/*gp2x_text_log("Wrong checksum of ROM");*/
					gp2x_text_log_fmt("Wrong checksum of ROM %s",name);
				checksumwarning++;
				/*if (expchecksum)*/
...
//	sprintf(sMsg, "Loading up to %d samples...", i);
//	gp2x_text_log(sMsg);


	if ((samples = (GameSamples*)gp2x_malloc(sizeof(struct GameSamples) + (i-1)*sizeof(struct GameSample))) == 0)
		return 0;
	...
	for (i = 0;i < samples->total;i++)
	{
		...
		{
			gp2x_text_log((char*)samplenames[i+skipfirst]);
			...
			if (f != 0)
			{
				if (osd_fseek(f,0,SEEK_END) == 0)
				{
					...
					if (memcmp(buf, "MAME", 4) == 0)
					{
						...
						osd_fread(f,&dummy,2);
						if ((smplen > 0) && (samples->sample[i] = (GameSample*)gp2x_malloc(sizeof(struct GameSample) + (smplen)*sizeof(char))) != 0)
						{
							if (smpres == 16)
								printf ("ERROR: 16-bit sample!\n");
//							{
//								char sMsg[40];
//								sprintf(sMsg,"Sample: l%d:f%d:r%d:v%d", smplen, smpfrq, (int)smpres, (int)smpvol);
//								gp2x_text_log(sMsg);
//							}
...
//							{
//								char sMsg[40];
//								sprintf(sMsg,"NOT LOAD Sample: l%d:f%d:r%d:v%d", smplen, smpfrq, (int)smpres, (int)smpvol);
//								gp2x_text_log(sMsg);
//							}
						}
					}
					else
						gp2x_text_log("Sample should be .SAM format");
...
struct GfxElement *decodegfx(const unsigned char *src,const struct GfxLayout *gl)
{
	...
	if ((gfx = (GfxElement*)gp2x_malloc(sizeof(struct GfxElement))) == 0) {
		return 0;
	}
	...
	{
		gfx->pen_usage = (unsigned int*)gp2x_malloc(gfx->total_elements * sizeof(int));
	}
...
#ifdef COMMON_TOTALS_PATCH
	while (code>=gfx->total_elements) code-=gfx->total_elements;
	while (color>=gfx->total_colors) color-=gfx->total_colors;
#else
				...
				if (flipx)	/* X flip */
				{
					for (y = sy;y <= ey;y++)
					{
						...
						if (!(((unsigned)sd4)&3))
						{
							...
							{
								if ((col4=read_dword_dword(sd4)) != trans4){
									col = (col4>>24)&0xff;
					...
					for (y = sy;y <= ey;y++)
					{
						...
						if (!(((unsigned)sd4)&3))
						{
							if (!(((unsigned)bm)&3))
							{
								...
								{
									/* bypass loop */
									while( bm <= bme-3 && read_dword_dword(sd4) == trans4 )
									{ sd4++; bm+=4; }
									/* drawing loop */
									while( bm <= bme-3 && (col4=read_dword_dword(sd4)) != trans4 )
									{
										xod4 = col4^trans4;
										if( (xod4&0x000000ff) && (xod4&0x0000ff00) &&
										    (xod4&0x00ff0000) && (xod4&0xff000000) )
										{
											write_dword_dword((int *)bm,col4);
										}
										else
										{
...
static unsigned short hacktable[256];
static int hacktable_inited=0;
		...
		const struct rectangle *clip,int transparency,int transparent_color)
{
	...
	drawgfx(dest,&mygfx,0,0,flipx,flipy,sx,sy,clip,transparency,transparent_color);	/* ASG 971011 */
}
...
void fillbitmap(struct osd_bitmap *dest,int pen,const struct rectangle *clip)
{
	...
	for (y = sy;y <= ey;y++)
		fast_memset(&dest->line[y][sx],pen,ex-sx+1);
}
