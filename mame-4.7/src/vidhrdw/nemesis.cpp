...
void nemesis_vh_stop(void)
{
	...
	tmpbitmap=0;
	gp2x_free (char_dirty);
	gp2x_free (sprite_dirty);
	gp2x_free (sprite3216_dirty);
	gp2x_free (sprite1632_dirty);
	gp2x_free (sprite3232_dirty);
	gp2x_free (sprite816_dirty);
	char_dirty = 0;
	gp2x_free (video1_dirty);
	gp2x_free (video2_dirty);
	gp2x_free (nemesis_characterram_gfx);
}
...
int nemesis_vh_start(void)
{
	...
	char_dirty = (unsigned char *)gp2x_malloc(2048);
	if (!char_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(char_dirty,1,2048);

	sprite_dirty = (unsigned char *)gp2x_malloc(512);
	if (!sprite_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(sprite_dirty,1,512);

	sprite3216_dirty = (unsigned char *)gp2x_malloc(256);
	if (!sprite3216_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(sprite3216_dirty,1,256);

	sprite1632_dirty = (unsigned char *)gp2x_malloc(256);
	if (!sprite1632_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(sprite1632_dirty,1,256);

	sprite3232_dirty = (unsigned char *)gp2x_malloc(128);
	if (!sprite3232_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(sprite3232_dirty,1,128);

	sprite816_dirty = (unsigned char *)gp2x_malloc(1024);
	if (!sprite816_dirty) {
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(sprite816_dirty,1,1024);

	video1_dirty = (unsigned char *)gp2x_malloc (0x800);
	video2_dirty = (unsigned char *)gp2x_malloc (0x800);
	if (!video1_dirty || !video2_dirty)
	{
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(video1_dirty,1,0x800);
	fast_memset(video2_dirty,1,0x800);

	nemesis_characterram_gfx = (unsigned char*)gp2x_malloc(nemesis_characterram_size);
	if(!nemesis_characterram_gfx)
	{
		nemesis_vh_stop();
		return 1;
	}
	fast_memset(nemesis_characterram_gfx,0,nemesis_characterram_size);
