...
{
	if ((wms_cmos_ram = (unsigned char *)gp2x_malloc(0x8000)) == 0)
	{
		return 1;
	}
	if ((paletteram = (unsigned char *)gp2x_malloc(0x4000)) == 0)
	{
		gp2x_free(wms_cmos_ram);
		return 1;
	}
	if ((wms_videoram = (unsigned short *)gp2x_malloc(wms_videoram_size)) == 0)
	{
		gp2x_free(wms_cmos_ram);
		gp2x_free(paletteram);
		return 1;
	}
	fast_memset(wms_cmos_ram,0,0x8000);
	return 0;
}
int wms_t_vh_start(void)
{
	...
	if ((wms_videoram = (unsigned short *)gp2x_malloc(0x100000)) == 0)
	{
		gp2x_free(wms_cmos_ram);
		gp2x_free(paletteram);
		return 1;
...
void wms_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	if (bitmap->depth==16)
	{
		...
		{																			
			...
			{																		
				fast_memcpy(rv-col-1,&wms_videoram[510*512],(col+1)*sizeof(unsigned short));	
			}																		
