...
{
	unsigned char *colors = (unsigned char *)param;
	int color = data[2] & 15;
	colors[color] = 1;
}
...
void eprom_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	fast_memset (mo_map, 0, sizeof (mo_map));
	fast_memset (pf_map, 0, sizeof (pf_map));
	fast_memset (al_map, 0, sizeof (al_map));
	...
	{
		if (pf_map[i])
			fast_memset (&palette_used_colors[512 + i * 16], PALETTE_COLOR_USED, 16);
		if (mo_map[i])
		{
			palette_used_colors[256 + i * 16] = PALETTE_COLOR_TRANSPARENT;
			fast_memset (&palette_used_colors[256 + i * 16 + 1], PALETTE_COLOR_USED, 15);
		}
		if (al_map[i])
			fast_memset (&palette_used_colors[0 + i * 4], PALETTE_COLOR_USED, 4);
		if (al_map[16+i])
			fast_memset (&palette_used_colors[0 + (i+32) * 4], PALETTE_COLOR_USED, 4);
	}
