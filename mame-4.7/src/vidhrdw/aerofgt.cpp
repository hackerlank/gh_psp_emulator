...
static int common_vh_start(int width,int height)
{
	...
	if ((dirtybuffer = (unsigned char *)gp2x_malloc(aerofgt_bg1videoram_size / 2)) == 0)
	{
		return 1;
	}
	fast_memset(dirtybuffer,1,aerofgt_bg1videoram_size / 2);
	...
	if ((dirtybuffer2 = (unsigned char *)gp2x_malloc(aerofgt_bg2videoram_size / 2)) == 0)
	{
		gp2x_free(dirtybuffer);
		osd_free_bitmap(tmpbitmap);
		return 1;
	}
	fast_memset(dirtybuffer2,1,aerofgt_bg2videoram_size / 2);
