...
int pang_vh_start(void)
{
	...
	*/
	pang_video_dirty=(unsigned char *)gp2x_malloc(pang_videoram_size*2);
	if (!pang_video_dirty)
	{
		pang_vh_stop();
		return 1;
	}
	fast_memset(pang_video_dirty, 1, pang_videoram_size);
	pang_video_dirty_base=pang_video_dirty;

	/*
		OBJ RAM
	*/
	pang_objram=(unsigned char *)gp2x_malloc(pang_videoram_size);
	if (!pang_objram)
	{
		pang_vh_stop();
		return 1;
	}
	fast_memset(pang_objram, 0, pang_videoram_size);

	/*
		Char colour dirty buffer
	*/
	pang_color_dirty=(unsigned char *)gp2x_malloc(pang_colorram_size);
	if (!pang_color_dirty)
	{
		pang_vh_stop();
		return 1;
	}
	fast_memset(pang_color_dirty, 1, pang_colorram_size);

	/*
		Palette RAM
	*/
	pang_paletteram=(unsigned char *)gp2x_malloc(pang_paletteram_size);
	if (!pang_paletteram)
	{
		pang_vh_stop();
		return 1;
	}
	fast_memset(pang_paletteram, 0, pang_paletteram_size);

	/*
		Palette dirty buffer
	*/
	pang_palette_dirty=(unsigned char *)gp2x_malloc(pang_paletteram_size);
	if (!pang_palette_dirty)
	{
		return 1;
	}
	fast_memset(pang_palette_dirty, 1, pang_paletteram_size);
...
void pang_vh_stop(void)
{
	...
	if (pang_video_dirty)
		gp2x_free(pang_video_dirty);
	if (pang_color_dirty)
		gp2x_free(pang_color_dirty);
	if (pang_objram)
		gp2x_free(pang_objram);
	if (pang_paletteram)
		gp2x_free(pang_paletteram);
	if (pang_palette_dirty)
		gp2x_free(pang_palette_dirty);
}
...
INLINE void pang_refresh_palette( void )
{
	...
	fast_memset(pang_used_colors,0, sizeof(pang_used_colors));
	...
	for (i=0; i<pang_max_colors; i++)
	{
		...
		{
			fast_memset(palette_used_colors+i*16, PALETTE_COLOR_USED, 16);
...
void pang_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	fast_memset(pang_dirty_colors, 0, sizeof(pang_dirty_colors));
