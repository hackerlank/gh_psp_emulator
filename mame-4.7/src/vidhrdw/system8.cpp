...
{
	if ((SpritesCollisionTable = (unsigned char *)gp2x_malloc(256*256)) == 0)
		return 1;
	fast_memset(SpritesCollisionTable,255,256*256);

	if ((bg_dirtybuffer = (unsigned char *)gp2x_malloc(1024)) == 0)
	{
		gp2x_free(SpritesCollisionTable);
		return 1;
	}
	fast_memset(bg_dirtybuffer,1,1024);
	if ((tx_dirtybuffer = (unsigned char *)gp2x_malloc(1024)) == 0)
	{
		gp2x_free(bg_dirtybuffer);
		gp2x_free(SpritesCollisionTable);
		return 1;
	}
	fast_memset(tx_dirtybuffer,1,1024);
	if ((bg_ram = (unsigned char *)gp2x_malloc(0x4000)) == 0)			/* Allocate 16k for background banked ram */
	{
		gp2x_free(bg_dirtybuffer);
		gp2x_free(tx_dirtybuffer);
		gp2x_free(SpritesCollisionTable);
		return 1;
	}
	fast_memset(bg_ram,0,0x4000);
...
static void Pixel(struct osd_bitmap *bitmap,int x,int y,int spr_number,int color)
{
	...
	if (SpritesCollisionTable[(y<<8)+x] == 255)
	{
		SpritesCollisionTable[(y<<8)+x] = spr_number;
		bitmap->line[adjy][adjx] = color;
	}
	else
	{
		SprOnScreen=SpritesCollisionTable[(y<<8)+x];
	...
	xr = ((x - background_scrollx) & 0xff) >> 3;
	yr = ((y - background_scrolly) & 0xff) >> 3;
	...
	{
		if (system8_backgroundram[(((yr<<5) + xr)<<1) + 1] & 0x10)
...
void system8_compute_palette (void)
{
	...
	fast_memset (bg_usage, 0, sizeof (bg_usage));
	fast_memset (tx_usage, 0, sizeof (tx_usage));
	fast_memset (sp_usage, 0, sizeof (sp_usage));
	...
	{
		if (bg_usage[i])
			fast_memset (palette_used_colors + 1024 + i * 8, PALETTE_COLOR_USED, 8);
		else
			fast_memset (palette_used_colors + 1024 + i * 8, PALETTE_COLOR_UNUSED, 8);

		palette_used_colors[512 + i * 8] = PALETTE_COLOR_TRANSPARENT;
		if (tx_usage[i])
			fast_memset (palette_used_colors + 512 + i * 8 + 1, PALETTE_COLOR_USED, 7);
		else
			fast_memset (palette_used_colors + 512 + i * 8 + 1, PALETTE_COLOR_UNUSED, 7);
	}
	...
	for (i = 0; i < 32; i++)
	{
		...
		if (sp_usage[i])
			fast_memset (palette_used_colors + 0 + i * 16 + 1, PALETTE_COLOR_USED, 15);
		else
			fast_memset (palette_used_colors + 0 + i * 16 + 1, PALETTE_COLOR_UNUSED, 15);
	}
	...
	else
	{
		...
		for (offs = 0;offs < system8_backgroundram_size;offs += 2)
		{
			if ((system8_backgroundram[offs+1] & 0x08) == priority)
			{
				...
				sx = sx<<3 + background_scrollx;
				sy = sy<<3 + background_scrolly;
