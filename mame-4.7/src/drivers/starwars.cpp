...
static int esb_slapstic_tweak(int offset)
{
	...
	if (bank != last_bank)
	{
		fast_memcpy(slapstic_area, &slapstic_base[bank * 0x2000], 0x2000);
		last_bank = bank;
	}
	return bank;
}
...
if (prevpc != 0x8080 && prevpc != 0x8090 && prevpc != 0x80a0 && prevpc !=0x80b0)
  bank = esb_slapstic_tweak ((prevpc) & 0x1fff);
	}
...
void empire_init_machine (void)
{
	...
	fast_memcpy(slapstic_area, &slapstic_base[slapstic_bank() * 0x2000], 0x2000);
...
INPUT_PORTS_START( esb_input_ports )
	...
	PORT_DIPSETTING(    0x00, "On" )
        ...
        PORT_START      /* DSW0 */
        PORT_DIPNAME(0x03, 0x03, "Starting Shields", IP_KEY_NONE )
        PORT_DIPSETTING (  0x01, "2" )
        ...
        PORT_DIPNAME(0x30, 0x30, "Jedi-Letter Mode", IP_KEY_NONE )
        ...
        PORT_DIPNAME(0x40, 0x40, "Demo_Sounds", IP_KEY_NONE )
        ...
        PORT_DIPSETTING (  0x02, "1C_1C" )
        PORT_DIPSETTING (  0x01, "1C_2C" )
        PORT_DIPSETTING (  0x00, "Free_Play" )
        PORT_DIPNAME(0x0c, 0x00, "Coin_B", IP_KEY_NONE )
        PORT_DIPSETTING (  0x00, "*1" )
        PORT_DIPSETTING (  0x04, "*4" )
        PORT_DIPSETTING (  0x08, "*5" )
        PORT_DIPSETTING (  0x0c, "*6" )
        PORT_DIPNAME(0x10, 0x00, "Coin_A", IP_KEY_NONE )
        PORT_DIPSETTING (  0x00, "*1" )
        PORT_DIPSETTING (  0x10, "*2" )
        PORT_DIPNAME(0xe0, 0xe0, "Bonus Coinage", IP_KEY_NONE )
