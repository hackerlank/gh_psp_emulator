#include "types.h"
#include "c68k.h"
#include <stdio.h>

/*
#ifdef NEOCD_HLE
void    neogeo_cdrom_load_files(void);
void    neogeo_cdda_control(void);
void    neogeo_prio_switch(void);
void    neogeo_upload(void);
#endif
*/

// exception cycle table (taken from musashi core)
static const s8 c68k_exception_cycle_table[256] =
{
	  4, //  0: Reset - Initial Stack Pointer
	  4, //  1: Reset - Initial Program Counter
	 50, //  2: Bus Error
	 50, //  3: Address Error
	 34, //  4: Illegal Instruction
	 38, //  5: Divide by Zero
	 40, //  6: CHK
	 34, //  7: TRAPV
	 34, //  8: Privilege Violation
	 34, //  9: Trace
	  4, // 10:
	  4, // 11:
	  4, // 12: RESERVED
	  4, // 13: Coprocessor Protocol Violation
	  4, // 14: Format Error
	 44, // 15: Uninitialized Interrupt
	  4, // 16: RESERVED
	  4, // 17: RESERVED
	  4, // 18: RESERVED
	  4, // 19: RESERVED
	  4, // 20: RESERVED
	  4, // 21: RESERVED
	  4, // 22: RESERVED
	  4, // 23: RESERVED
	 44, // 24: Spurious Interrupt
	 44, // 25: Level 1 Interrupt Autovector
	 44, // 26: Level 2 Interrupt Autovector
	 44, // 27: Level 3 Interrupt Autovector
	 44, // 28: Level 4 Interrupt Autovector
	 44, // 29: Level 5 Interrupt Autovector
	 44, // 30: Level 6 Interrupt Autovector
	 44, // 31: Level 7 Interrupt Autovector
	 34, // 32: TRAP #0
	 34, // 33: TRAP #1
	 34, // 34: TRAP #2
	 34, // 35: TRAP #3
	 34, // 36: TRAP #4
	 34, // 37: TRAP #5
	 34, // 38: TRAP #6
	 34, // 39: TRAP #7
	 34, // 40: TRAP #8
	 34, // 41: TRAP #9
	 34, // 42: TRAP #10
	 34, // 43: TRAP #11
	 34, // 44: TRAP #12
	 34, // 45: TRAP #13
	 34, // 46: TRAP #14
	 34, // 47: TRAP #15
	  4, // 48: FP Branch or Set on Unknown Condition
	  4, // 49: FP Inexact Result
	  4, // 50: FP Divide by Zero
	  4, // 51: FP Underflow
	  4, // 52: FP Operand Error
	  4, // 53: FP Overflow
	  4, // 54: FP Signaling NAN
	  4, // 55: FP Unimplemented Data Type
	  4, // 56: MMU Configuration Error
	  4, // 57: MMU Illegal Operation Error
	  4, // 58: MMU Access Level Violation Error
	  4, // 59: RESERVED
	  4, // 60: RESERVED
	  4, // 61: RESERVED
	  4, // 62: RESERVED
	  4, // 63: RESERVED
	     // 64-255: User Defined
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,
	  4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4
};

extern unsigned char m68ki_cycles[][0x10000];

u32 m68k_int_masks[] = {0xfe, 0xfc, 0xf8, 0xf0, 0xe0, 0xc0, 0x80, 0x80};


// global variable
///////////////////

//#ifndef C68K_CONST_JUMP_TABLE
//static void *JumpTable[0x10000];
//#endif

static u32 c68k_shiftop_mask[64][2];
static u32 C68k_Initialised = 0;

/*
#ifdef NEOCD_HLE
extern int img_display;
#endif
*/

// include macro file
//////////////////////

c68k_struc *CPU;
u32 PC     ;
s32 CCnt   ;
u32 Opcode ;
int bStop = 0 ;

/*
#define MAX_DEBUG_OP  (10*1024*1024)
u32 op_debug[MAX_DEBUG_OP];
int op_debug_num = 0 ;
int op_init = 0;
int op_debug_num_e ;
unsigned long op_debug_e ;
//*/

#include "c68kmac.inc"

#include "c68k_op0.inc"
#include "c68k_op1.inc"
#include "c68k_op2.inc"
#include "c68k_op3.inc"
#include "c68k_op4.inc"
#include "c68k_op5.inc"
#include "c68k_op6.inc"
#include "c68k_op7.inc"
#include "c68k_op8.inc"
#include "c68k_op9.inc"
#include "c68k_opA.inc"
#include "c68k_opB.inc"
#include "c68k_opC.inc"
#include "c68k_opD.inc"
#include "c68k_opE.inc"

void FASTCALL C68k_Use_Cycle(c68k_struc *cpu, s32 cycle)
{
	CCnt -= cycle ;
}


void (*JumpTable[0x10000])(void) ;
//extern void  (*m68ki_instruction_jump_table[0x10000])(void); /* opcode handler jump table */
//#define JumpTable m68ki_instruction_jump_table

void C68k_InitTable( int force )
{
	int i , j ;

	if( (force == 0) && C68k_Initialised ){ return ; }

#include "c68k_ini.inc"

	C68k_Initialised = 1 ;
}

#ifdef _DEBUG
//#define _DEBUG_C68K
#endif //

#ifdef  _DEBUG_C68K
#define MAX_DATA_CT  100000
extern unsigned short data   [2][ MAX_DATA_CT ] ;
extern unsigned short clkdata[2][ MAX_DATA_CT ] ;
extern int            data_ct[2] ;
#endif // _DEBUG_C68K

// main exec function
//////////////////////

s32 FASTCALL C68k_Exec(c68k_struc *cpu, s32 cycle)
{
    CPU = cpu;
    PC = CPU->PC;

    if (CPU->Status & (C68K_RUNNING | C68K_DISABLE | C68K_FAULTED))
    {
        return (CPU->Status | 0x80000000);
    }

    //if (cycle <= 0) return -cycle;
    if (cycle <= 0) return 0 ;
    
    CPU->CycleToDo = CCnt = cycle;

	/*
#ifndef C68K_DEBUG
    CHECK_INT
#else
    {
        s32 line, vect ;

        line = CPU->IRQLine;
	   //if ((1 << line) & m68k_int_masks[CPU->flag_I])
        if ((line == 7) || (line > CPU->flag_I))
        {
#ifdef  _DEBUG_C68K
			if( data_ct[1] < 10000 ){ data[1][ data_ct[1]++ ] = -1 ; }
#endif // _DEBUG_C68K

            PRE_IO

            CPU->IRQLine = 0;
            vect = CPU->Interrupt_CallBack(line);
            if (vect == C68K_INT_ACK_AUTOVECTOR)
                vect = C68K_INTERRUPT_AUTOVECTOR_EX + (line & 7);

            CCnt -= c68k_exception_cycle_table[vect];

            if (!CPU->flag_S)
            {
                u32 tmpSP;

                tmpSP = CPU->USP;
                CPU->USP = CPU->A[7];
                CPU->A[7] = tmpSP;
            }

            PUSH_32_F(PC - CPU->BasePC)
            PUSH_16_F(GET_SR)

            CPU->flag_S = C68K_SR_S;
            CPU->flag_I = line;

            READ_LONG_F(vect * 4, PC)
            SET_PC(PC)

            POST_IO
        }
    }
#endif
	*/

	//CPU->IRQLine = 0; // Genesis only!!

    if (CPU->Status & (C68K_HALTED | C68K_WAITING)) return CPU->CycleToDo;

    CPU->CycleSup = 0;
    CPU->Status |= C68K_RUNNING;

C68k_Exec:


#ifndef C68K_DEBUG
    CHECK_INT
#else
    {
        s32 line, vect ;

        line = CPU->IRQLine;
	   //if ((1 << line) & m68k_int_masks[CPU->flag_I])
        if ((line == 7) || (line > CPU->flag_I))
        {

#ifdef  _DEBUG_C68K
			//if( data_ct[1] < MAX_DATA_CT ){ clkdata[1][data_ct[1]] = CCnt; data[1][ data_ct[1]++ ] = -1 ; }
			//if( data_ct[1] < MAX_DATA_CT ){ clkdata[1][data_ct[1]] = CCnt; data[1][ data_ct[1]++ ] = line ; }
#endif // _DEBUG_C68K

            PRE_IO

            /* get vector */
            CPU->IRQLine = 0;
            vect = CPU->Interrupt_CallBack(line);
            if (vect == C68K_INT_ACK_AUTOVECTOR)
                vect = C68K_INTERRUPT_AUTOVECTOR_EX + (line & 7);

            /* adjust CCnt */
            //CCnt -= c68k_exception_cycle_table[vect];

           /* swap A7 and USP */
            if (!CPU->flag_S)
            {
                u32 tmpSP;

                tmpSP = CPU->USP;
                CPU->USP = CPU->A[7];
                CPU->A[7] = tmpSP;
            }

            /* push PC and SR */
            PUSH_32_F(PC - CPU->BasePC)
            PUSH_16_F(GET_SR)

            /* adjust SR */
            CPU->flag_S = C68K_SR_S;
            CPU->flag_I = line;

            /* fetch new PC */
            READ_LONG_F(vect * 4, PC)
            SET_PC(PC)

            POST_IO
        }
    }
#endif

#ifndef C68K_DEBUG
    NEXT

#else
    Opcode = FETCH_WORD;        
	PC += 2;
			
	JumpTable[Opcode]() ;  
	//CCnt -= m68ki_cycles[0][ Opcode ] ;

#ifdef  _DEBUG_C68K
	if( data_ct[1] < MAX_DATA_CT ){ clkdata[1][data_ct[1]] = PC - CPU->BasePC ; data[1][ data_ct[1]++ ] = Opcode ; }

	if( data[1][ data_ct[1] -1 ] != data[0][ data_ct[1] -1 ] )
	{
		Opcode = 0 ;
		//data[1][ data_ct[1] -1 ] = data[0][ data_ct[1] -1 ] ;
	}
#endif // _DEBUG_C68K

	//if( (CCnt <= 0) || bStop ){ bStop=0; goto C68k_Exec_End ;} 
	if( (CCnt <= 0) ){ bStop=0; goto C68k_Exec_End ;} 
	else                       goto C68k_Exec ;    
	
#endif


C68k_Exec_End:

	//*
    if ((CCnt += CPU->CycleSup) > 0)
    {
        CPU->CycleSup = 0;
        //NEXT;
		goto C68k_Exec ;
    }
	//*/


C68k_Exec_Really_End:
    CPU->Status &= ~C68K_RUNNING;
    CPU->PC = PC;

#ifdef  _DEBUG_C68K
	//if( data_ct[1] < MAX_DATA_CT ){ clkdata[1][data_ct[1]] = CCnt; data[1][ data_ct[1]++ ] = 0xFFF1 ; }
#endif // _DEBUG_C68K
    
    return (CPU->CycleToDo - CCnt);
}
