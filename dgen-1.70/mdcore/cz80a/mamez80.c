...
#define change_pc(pc)
#define cpu_readop(pc)		neogeo_z80_memory[pc]
#define cpu_readop_arg(pc)	neogeo_z80_memory[pc]
...
#define IN(port)   ((UINT8)neogeo_z80_port_r(port))

/***************************************************************
 * Output a byte to given I/O port
 ***************************************************************/
//#define OUT(port,value) io_write_byte_8(port,value)
#define OUT(port,value) neogeo_z80_port_w(port,value)

/***************************************************************
 * Read a byte from given memory location
 ***************************************************************/
//#define RM(addr) (UINT8)program_read_byte_8(addr)
//#define RM(addr) (UINT8)z80_read_memory_8(addr)
#define RM(addr)	neogeo_z80_memory[addr]
...
//#define WM(addr,value) z80_write_memory_8(addr,value)
#define WM(addr,value)	neogeo_z80_memory[addr] = (value)
...
OP(illegal,1)
{
	LOG(("Z80 ill. opcode $%02x $%02x\n", cpu_readop((PCD-1)&0xffff), cpu_readop(PCD)));
}
...
void z80_init(void)
{
	...
	if( !SZHVC_add || !SZHVC_sub )
	{
		...
		{
#if JAPANESE_UI
			fatalerror("Z80のフラグ用配列を確保できません。");
#else
			fatalerror("Could not allocate Z80 flags arrays.");
			return;
#endif
		}
...
int z80_execute(int cycles)
{
	if (cpu_enable)
	{
		z80_InitialICount = z80_ICount = cycles - Z80.extra_cycles;
		Z80.extra_cycles = 0;
...
void state_save_z80(u8 *state_buf)
{
	...
	state_save_byte(&after_EI, 1);
	state_save_byte(&Z80.halt, 1);
	state_save_long(&z80_ICount, 1);
	state_save_long(&z80_InitialICount, 1);
	state_save_long(&Z80.extra_cycles, 1);
}
...
void state_load_z80(u8 *state_buf)
{
	...
	state_load_byte(&after_EI, 1);
	state_load_byte(&Z80.halt, 1);
	state_load_long(&z80_ICount, 1);
	state_load_long(&z80_InitialICount, 1);
	state_load_long(&Z80.extra_cycles, 1);
