...
//int     num_t[0x10000] ;
//unsigned long op_debug[0xFF];
//int op_debug_num = 0 ;

#define MAX_DEBUG_OP  (10*1024*1024)
unsigned long op_debug[MAX_DEBUG_OP];
int op_debug_num = 0 ;
...
int m68k_execute(int num_clks)
{
   ...
   if(!CPU_STOPPED)
   {
	  ...
	  if( op_init == 0 )
	  {
		...
		if( p )
		{
			op_init = fread( op_debug, sizeof(op_debug),1, p ) ;
			fclose( p ) ;
		}
	  }
	  */
...
*/
		 ...
		 /*
		 if( op_debug[op_debug_num] != CPU_IR )
		 {
			 op_debug_e     = op_debug[op_debug_num] ;
			 op_debug_num_e = op_debug_num ;
			 op_debug_num ++ ;
		 }
		 op_debug[op_debug_num++] = m68ki_read_32( CPU_A[7] ) ;



		if( op_debug_num >= MAX_DEBUG_OP )
		{ 
			FILE* p ;
			p = fopen( "debug8.dat", "w" ) ;
			if( p )
			{
				fwrite( op_debug, sizeof(op_debug),1, p ) ;
				fclose( p ) ;
			}
			op_debug_num = MAX_DEBUG_OP ; 
		}
		 op_debug[op_debug_num++] = CPU_IR ;

		if( op_debug_num == 0x1A95C0/4 )
		{
			op_debug_num_e = op_debug_num ;
		}
	op_debug_num++ ;

	if( op_debug_num > 0x700000 )
	{
		FILE* p ;
		p = fopen( "debug10.dat", "w" ) ;
		if( p )
		{
			fwrite( op_debug, sizeof(op_debug),1, p ) ;
			fclose( p ) ;
	...
	else if( op_debug_num > 0x600000 )
	{
		op_debug[ op_debug_num & (MAX_DEBUG_OP-1) ] = CPU_IR ;
	}

	op_debug_num++ ;
	if( op_debug_num == 0x198544 )
	{
		op_debug_num = 0x198544 ;
	}

	op_debug[ op_debug_num++ ] = m68k_clks_left ;
	if( op_debug_num >= MAX_DEBUG_OP )
	{ 
		FILE* p ;
		p = fopen( "debug3.dat", "w" ) ;
		if( p )
		{
			fwrite( op_debug, sizeof(op_debug),1, p ) ;
			fclose( p ) ;
