...
unsigned int m68k_get_reg(void* context, m68k_register_t regnum)
{
	...
	switch(regnum)
	{
		...
		case M68K_REG_T0: return cpu->t0_flag    ;
		case M68K_REG_T1: return cpu->t1_flag    ;
		case M68K_REG_SF: return cpu->s_flag     ;
		case M68K_REG_MF: return cpu->m_flag     ;
		case M68K_REG_XF: return cpu->x_flag     ;
		case M68K_REG_NF: return cpu->n_flag     ;
		case M68K_REG_ZF: return cpu->not_z_flag ;
		case M68K_REG_VF: return cpu->v_flag     ;
		case M68K_REG_CF: return cpu->c_flag     ;
		case M68K_REG_IM: return cpu->int_mask   ;
...
void m68k_set_reg(m68k_register_t regnum, unsigned int value)
{
	switch(regnum)
	{
		...
		case M68K_REG_T0: FLAG_T0       = value ; return ;
		case M68K_REG_T1: FLAG_T1       = value ; return ;
		case M68K_REG_SF: //FLAG_S        = 
			...
			return ;
		case M68K_REG_MF: FLAG_M        = value ; return ;
		case M68K_REG_XF: FLAG_X        = value ; return ;
		case M68K_REG_NF: FLAG_N        = value ; return ;
		case M68K_REG_ZF: FLAG_Z        = value ; return ;
		case M68K_REG_VF: FLAG_V        = value ; return ;
		case M68K_REG_CF: FLAG_C        = value ; return ;
		case M68K_REG_IM: FLAG_INT_MASK = value ; return ;
...
#ifdef  _DEBUG_M68K
#define MAX_DATA_CT  100000
extern unsigned short data   [2][ MAX_DATA_CT ] ;
extern unsigned short clkdata[2][ MAX_DATA_CT ] ;
extern int            data_ct[2] ;
#endif // _DEBUG_M68K
...
#ifdef  _DEBUG_M68K
			if( data_ct[0] < MAX_DATA_CT ){ clkdata[0][data_ct[0]] = REG_PC ; data[0][ data_ct[0]++ ] = REG_IR ; }
#endif // _DEBUG_M68K
...
#ifdef  _DEBUG_M68K
			//if( data_ct[0] < MAX_DATA_CT ){ clkdata[0][data_ct[0]] = GET_CYCLES(); data[0][ data_ct[0]++ ] = 0xFFF1 ; }
#endif // _DEBUG_M68K
...
void m68k_use_cycles( int cycles )
{
	USE_CYCLES( cycles ) ;
}
...
#ifdef  _DEBUG_M68K
	//if( data_ct[0] < MAX_DATA_CT ){ clkdata[0][data_ct[0]] = GET_CYCLES(); data[0][ data_ct[0]++ ] = -1 ; }
	//if( int_level > 0 && data_ct[0] < MAX_DATA_CT ){ clkdata[0][data_ct[0]] = GET_CYCLES(); data[0][ data_ct[0]++ ] = int_level ; }
#endif // _DEBUG_M68K
}
