...
void GPU_ligne(NDS_Screen * screen, u16 l)
{
	...
	// (under DISP_MMEM_FIFO)
			for (i=0; i<256;) {
				c = IPC_FIFOrecv(MMU.fifos + MAIN_MEMORY_DISP_FIFO);//FIFOValue(MMU.fifos + MAIN_MEMORY_DISP_FIFO);
				...
				switch(capcnt->Capture_Source)
				{
					case 0:	/* source A only */
						if(capcnt->Source_A == 1)	/* capture 3D only */
						{
							...
							gpu3D->NDS_3D_GetLineCaptured(l, (u16*)cap3DLine);	/*FIXME: not sure it's good, since I hadn't seen how 3D works in desmume */
