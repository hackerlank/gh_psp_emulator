...
int polyisTranslucent ()
	{
		POLY *myself;
		//alpha != 31 -> translucent
		if((myself->polyAttr & 0x001F0000) != 0x001F0000) 
			return true;
		int texFormat = (myself->texParam>>26)&7;
...
GFX3D* GFX3D_create ()
{
  ...
  if (p == NULL) return (p) ;
   ...
   p->width = 256 ;
   p->height = 256 ;
   p->enableTexturing = true ;
	p->enableAlphaTest = true ;
	p->enableAlphaBlending = true ;
	p->enableAntialiasing = false ;
    p->enableEdgeMarking = false ;
	p->shading = TOON ;
	p->polylist = 0 ;
	p->vertlist = 0 ;
	p->alphaTestRef = 0 ;
  ...
  p->clearColor[0] = p->clearColor[1] = p->clearColor[2] =
    //threadclearColor[3] = 0;
  p->fogColor[0] = p->fogColor[1] = p->fogColor[2] = p->fogColor[3] = 0;
  p->fogOffset = 0 ;
...
void gfx3d_reset()
{
	gfx3d = malloc (sizeof (gfx3d));//GFX3D_create ();
...
void gfx3d_glViewPort(unsigned long v)
{
	...
	gfx3d->x = (v&0xFF);
	gfx3d->y = (v&0xFF);
	gfx3d->width = (((v>>16)&0xFF)+1)-(v&0xFF);
	gfx3d->height = ((v>>24)+1)-((v>>8)&0xFF);
}
...
{
	float coordTransformed[4] = { coord[0], coord[1], coord[2], 1 };
	...
	//apply lighting model
	{
		...
		for(i=0;i<4;i++) {
...

			{
				...
				float diffuseLevel = max(0.0f, -vec3dot(cacheLightDirection[i], normal));
				float shininessLevel = pow(max(0.0f, vec3dot(-cacheHalfVector[i], normal)), 2);
		...
		for(c=0;c<3;c++)
			colorRGB[c] = min(31,vertexColor[c]);
	}
}
...
static void NOPARAMS(u32 val)
{
	for (;;)
	{
		switch (clCmd & 0xFF)
		{
			...
			case 0x11:
				{
					*(u16 *)(ARM9Mem.ARM9_REG + 0x444) = val;
					gfx3d_glPushMatrix();
					GFX_FIFOsend(clCmd & 0xFF, val);
					clCmd >>= 8;
					clInd--;
			...
			case 0x15:
				{
					*(u16 *)(ARM9Mem.ARM9_REG + 0x454) = val;
					gfx3d_glLoadIdentity();
					GFX_FIFOsend(clCmd & 0xFF, val);
					clCmd >>= 8;
					clInd--;
			...
			case 0x41:
				{
					*(u16 *)(ARM9Mem.ARM9_REG + 0x504) = val;
					gfx3d_glEnd();
					GFX_FIFOsend(clCmd & 0xFF, val);
					clCmd >>= 8;
					clInd--;
...
void gfx3d_sendCommandToFIFO(u32 val)
{
	...
	{
		case 0x10:		// MTX_MODE - Set Matrix Mode (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x440) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x12:		// MTX_POP - Pop Current Matrix from Stack (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x448) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x13:		// MTX_STORE - Store Current Matrix on Stack (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x44C) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x14:		// MTX_RESTORE - Restore Current Matrix from Stack (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x450) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x16:		// MTX_LOAD_4x4 - Load 4x4 Matrix to Current Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x458) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x17:		// MTX_LOAD_4x3 - Load 4x3 Matrix to Current Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x45C) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x18:		// MTX_MULT_4x4 - Multiply Current Matrix by 4x4 Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x460) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x19:		// MTX_MULT_4x3 - Multiply Current Matrix by 4x3 Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x464) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x1A:		// MTX_MULT_3x3 - Multiply Current Matrix by 3x3 Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x468) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x1B:		// MTX_SCALE - Multiply Current Matrix by Scale Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x46C) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x1C:		// MTX_TRANS - Mult. Curr. Matrix by Translation Matrix (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x470) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x20:		// COLOR - Directly Set Vertex Color (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x480) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
			gfx3d_glColor3b(val);
			clCmd >>= 8;
			clInd--;
		break;
		case 0x21:		// NORMAL - Set Normal Vector (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x484) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
			gfx3d_glNormal(val);
			clCmd >>= 8;
			clInd--;
		break;
		case 0x22:		// TEXCOORD - Set Texture Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x488) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x23:		// VTX_16 - Set Vertex XYZ Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x48C) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x24:		// VTX_10 - Set Vertex XYZ Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x490) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x25:		// VTX_XY - Set Vertex XY Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x494) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x26:		// VTX_XZ - Set Vertex XZ Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x498) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x27:		// VTX_YZ - Set Vertex YZ Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x49C) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x28:		// VTX_DIFF - Set Relative Vertex Coordinates (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4A0) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x29:		// POLYGON_ATTR - Set Polygon Attributes (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4A4) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x2A:		// TEXIMAGE_PARAM - Set Texture Parameters (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4A8) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x2B:		// PLTT_BASE - Set Texture Palette Base Address (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4AC) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x30:		// DIF_AMB - MaterialColor0 - Diffuse/Ambient Reflect. (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4C0) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x31:		// SPE_EMI - MaterialColor1 - Specular Ref. & Emission (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4C4) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x32:		// LIGHT_VECTOR - Set Light's Directional Vector (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4C8) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x33:		// LIGHT_COLOR - Set Light Color (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4CC) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x34:		// SHININESS - Specular Reflection Shininess Table (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x4D0) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x40:		// BEGIN_VTXS - Start of Vertex List (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x500) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
			gfx3d_glBegin(val);
			clCmd >>= 8;
			clInd--;
		break;
		case 0x50:		// SWAP_BUFFERS - Swap Rendering Engine Buffer (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x540) = val;
			gfx3d_glFlush(val);
		...
		break;
		case 0x60:		// VIEWPORT - Set Viewport (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x580) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x70:		// BOX_TEST - Test if Cuboid Sits inside View Volume (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x5C0) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x71:		// POS_TEST - Set Position Coordinates for Test (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x5C4) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
		...
		break;
		case 0x72:		// VEC_TEST - Set Directional Vector for Test (W)
			*(u16 *)(ARM9Mem.ARM9_REG + 0x5C8) = val;
			GFX_FIFOsend(clCmd & 0xFF, val);
			gfx3d_glVecTest(val);
			clCmd >>= 8;
			clInd--;
...
void gfx3d_sendCommand(u32 cmd, u32 param)
{
	//u32 gxstat = (*(u16 *)(MMU.MMU_MEM[ARMCPU_ARM9][0x40]))[0x600>>2];
...
static void gfx3d_Control_cache()
{
	...
	if((v>>1)&1) gfx3d->shading = HIGHLIGHT;
	else gfx3d->shading = TOON;
