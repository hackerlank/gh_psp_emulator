...
{
	if(g_nRenderMsgCounter==1 || g_nRenderMsgCounter==2 || g_nRenderMsgCounter == (g_nRenderMsgCount-1) || g_nRenderMsgCounter==g_nRenderMsgCount) pgFillBox(0,263,479,271,0);
	...
	if(g_nRenderMsgCounter>0) g_nRenderMsgCounter--;
	// draw fps
	if (setting.showfps) {
...
static void blt_gpu(int scr_w,int scr_h,void *XBuf,int gfx_sx,int gfx_sy,int gfx_w,int gfx_h)
{
	...
	//>>>davex
	 if( screen_rotated_on){
    	...
    	vertex[0].x = ScreenX;          
    	vertex[0].y = ScreenY;          
    	vertex[1].x = ScreenX+scr_w;    
    	vertex[1].y = ScreenY+scr_h;    
