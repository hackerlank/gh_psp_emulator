...
typedef void (*InstrHandlerFuncType)(my_int16 Operand);
typedef my_int16 (*AddrModeHandlerFuncType)(int WantsAddr);
...
/*----------------------------------------------------------------------------*/
INLINE static void PushWord(my_int16 topush) {
  Push((topush>>8) & 255);
  Push(topush & 255);
} /* PushWord */

/*----------------------------------------------------------------------------*/
INLINE static my_int16 PopWord() {
  my_int16 RetValue;
...
INLINE static my_int16 RelAddrModeHandler_Data(void) {
...
/*----------------------------------------------------------------------------*/
INLINE static void ADCInstrHandler(my_int16 operand) {
...
/*----------------------------------------------------------------------------*/
INLINE static void ANDInstrHandler(my_int16 operand) {
  Accumulator=Accumulator & operand;
  PSR&=~(FlagZ | FlagN);
  PSR|=((Accumulator==0)<<1) | (Accumulator & 128);
} /* ANDInstrHandler */

INLINE static void ASLInstrHandler(my_int16 address) {
...
INLINE static void TRBInstrHandler(my_int16 address) {
	unsigned char oldVal,newVal;
	oldVal=TUBEREADMEM_FAST(address);
...
INLINE static void TSBInstrHandler(my_int16 address) {
	unsigned char oldVal,newVal;
	oldVal=TUBEREADMEM_FAST(address);
...
INLINE static void BITInstrHandler(my_int16 operand) {
...
INLINE static void CMPInstrHandler(my_int16 operand) {
...
INLINE static void EORInstrHandler(my_int16 operand) {
  Accumulator^=operand;
  SetPSRZN(Accumulator);
} /* EORInstrHandler */

INLINE static void INCInstrHandler(my_int16 address) {
  unsigned char val;
...
INLINE static void JSRInstrHandler(my_int16 address) {
  PushWord(TubeProgramCounter-1);
  TubeProgramCounter=address;
} /* JSRInstrHandler */

INLINE static void LDAInstrHandler(my_int16 operand) {
  Accumulator=operand;
  SetPSRZN(Accumulator);
} /* LDAInstrHandler */

INLINE static void LDXInstrHandler(my_int16 operand) {
  XReg=operand;
  SetPSRZN(XReg);
} /* LDXInstrHandler */

INLINE static void LDYInstrHandler(my_int16 operand) {
  YReg=operand;
  SetPSRZN(YReg);
} /* LDYInstrHandler */

INLINE static void LSRInstrHandler(my_int16 address) {
  unsigned char oldVal,newVal;
  oldVal=TUBEREADMEM_FAST(address);
...
INLINE static void ORAInstrHandler(my_int16 operand) {
  Accumulator=Accumulator | operand;
  SetPSRZN(Accumulator);
} /* ORAInstrHandler */

INLINE static void ROLInstrHandler(my_int16 address) {
  unsigned char oldVal,newVal;
...
INLINE static void RORInstrHandler(my_int16 address) {
  unsigned char oldVal,newVal;
...
INLINE static void SBCInstrHandler(my_int16 operand) {
  /* NOTE! Not sure about C and V flags */
  int TmpResultV,TmpResultC;
...
INLINE static void STXInstrHandler(my_int16 address) {
  TUBEWRITEMEM_FAST(address,XReg);
} /* STXInstrHandler */

INLINE static void STYInstrHandler(my_int16 address) {
...
INLINE static my_int16 AbsAddrModeHandler_Data(void) {
  int FullAddress;
...
/*-------------------------------------------------------------------------*/
/* Absolute  addressing mode handler                                       */
INLINE static my_int16 AbsAddrModeHandler_Address(void) {
  int FullAddress;
...
INLINE static my_int16 ZeroPgAddrModeHandler_Address(void) {
  return(TubeRam[TubeProgramCounter++]);
} /* ZeroPgAddrModeHandler_Address */

/*-------------------------------------------------------------------------*/
/* Indexed with X preinc addressing mode handler                           */
INLINE static my_int16 IndXAddrModeHandler_Data(void) {
...
/*-------------------------------------------------------------------------*/
/* Indexed with X preinc addressing mode handler                           */
INLINE static my_int16 IndXAddrModeHandler_Address(void) {
  unsigned char ZeroPageAddress;
  int EffectiveAddress;
...
INLINE static my_int16 IndYAddrModeHandler_Data(void) {
  int EffectiveAddress;
...
/*-------------------------------------------------------------------------*/
/* Indexed with Y postinc addressing mode handler                          */
INLINE static my_int16 IndYAddrModeHandler_Address(void) {
  int EffectiveAddress;
  unsigned char ZPAddr=TubeRam[TubeProgramCounter++];
  EffectiveAddress=TubeRam[ZPAddr]+YReg;
...
INLINE static my_int16 ZeroPgXAddrModeHandler_Data(void) {
  int EffectiveAddress;
...
/*-------------------------------------------------------------------------*/
/* Zero page wih X offset addressing mode handler                          */
INLINE static my_int16 ZeroPgXAddrModeHandler_Address(void) {
  int EffectiveAddress;
  EffectiveAddress=(TubeRam[TubeProgramCounter++]+XReg) & 255;
  return(EffectiveAddress);
} /* ZeroPgXAddrModeHandler */
...
INLINE static my_int16 AbsXAddrModeHandler_Data(void) {
  int EffectiveAddress;
...
/*-------------------------------------------------------------------------*/
/* Absolute with X offset addressing mode handler                          */
INLINE static my_int16 AbsXAddrModeHandler_Address(void) {
  int EffectiveAddress;
  GETTWOBYTEFROMPC(EffectiveAddress)
...
INLINE static my_int16 AbsYAddrModeHandler_Data(void) {
  int EffectiveAddress;
  GETTWOBYTEFROMPC(EffectiveAddress);
...
/*-------------------------------------------------------------------------*/
/* Absolute with Y offset addressing mode handler                          */
INLINE static my_int16 AbsYAddrModeHandler_Address(void) {
  int EffectiveAddress;
  GETTWOBYTEFROMPC(EffectiveAddress)
...
INLINE static my_int16 IndAddrModeHandler_Address(void) {
...
INLINE static my_int16 ZPIndAddrModeHandler_Address(void) {
  int VectorLocation;
  int EffectiveAddress;
...
/*-------------------------------------------------------------------------*/
/* Zero page Indirect addressing mode handler                                        */
INLINE static my_int16 ZPIndAddrModeHandler_Data(void) {
  int VectorLocation;
  int EffectiveAddress;
...
INLINE static my_int16 IndAddrXModeHandler_Address(void) {
  /* For jump indirect only */
  int VectorLocation;
  int EffectiveAddress;
...
INLINE static my_int16 ZeroPgYAddrModeHandler_Data(void) {
  int EffectiveAddress;
...
/*-------------------------------------------------------------------------*/
/* Zero page with Y offset addressing mode handler                         */
INLINE static my_int16 ZeroPgYAddrModeHandler_Address(void) {
  int EffectiveAddress;
  EffectiveAddress=(TubeRam[TubeProgramCounter++]+YReg) & 255;
  return(EffectiveAddress);
} /* ZeroPgYAddrModeHandler */
...
void Exec65C02Instruction(void) {
  ...
  switch (CurrentInstruction) {
    ...
    case 0x07: /* Undocumented Instruction: ASL zp and ORA zp */
      {
        my_int16 zpaddr = ZeroPgAddrModeHandler_Address();
    ...
    case 0x03: /* Undocumented Instruction: ASL-ORA (zp,X) */
      {
        my_int16 zpaddr = IndXAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
    case 0x13: /* Undocumented Instruction: ASL-ORA (zp),Y */
      {
        my_int16 zpaddr = IndYAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
    case 0x0f: /* Undocumented Instruction: ASL-ORA abs */
      {
        my_int16 zpaddr = AbsAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
    case 0x17: /* Undocumented Instruction: ASL-ORA zp,X */
      {
        my_int16 zpaddr = ZeroPgXAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
    case 0x1b: /* Undocumented Instruction: ASL-ORA abs,Y */
      {
        my_int16 zpaddr = AbsYAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
    case 0x1f: /* Undocumented Instruction: ASL-ORA abs,X */
      {
        my_int16 zpaddr = AbsXAddrModeHandler_Address();
        ASLInstrHandler(zpaddr);
        ORAInstrHandler(TubeRam[zpaddr]);
      }
      break;
