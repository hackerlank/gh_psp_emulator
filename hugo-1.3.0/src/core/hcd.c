...
static void
my_Mix_PlayMusic(int i, int loops)
{
# if 0 //ZX:
  if (! sdlmixmusic[i]) {
    sdlmixmusic[i] = Mix_LoadWAV(CD_track[i].filename);
  }
  Mix_PlayMusic(sdlmixmusic[i], loops);
# endif
}
...
HCD_play_track (UChar track, char repeat)
{
  ...
  if ((CD_track[track].source_type == HCD_SOURCE_REGULAR_FILE) &&
    ...
    {
      my_Mix_PlayMusic(track,repeat);
      MP3_playing = 1;
    }
#elif defined (NGC) || defined(PSP)
#endif
};
void
...
# endif
  ...
  if ((CD_track[result].source_type == HCD_SOURCE_REGULAR_FILE) &&
   ...
   frame - (begin_sect - CD_track[result].beg_lsn) / 75.0 * 60.0;
      ...
      if (-150 < begin_sect - CD_track[result].beg_lsn < 150)
        my_Mix_PlayMusic(result, repeat);
      else       /* can't yet easily repeat "inside" a track */
        my_Mix_PlayMusic(result, FALSE);
